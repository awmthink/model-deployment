...
void __checkCudaErrors(cudaError_t result, char const *const func, const char *const file,
                       int const line) {
...
void __checkCudaErrors(cublasStatus_t result, char const *const func, const char *const file,
                       int const line) {
  if (result != CUBLAS_STATUS_SUCCESS) {
    fprintf(stderr, "CUBLAS error at %s:%d code=%u \"%s\" \n", file, line,
...
inline void __checkLastCudaError(const char *file, const int line) {
  ...
  if (cudaSuccess != err) {
    fprintf(stderr, "%s(%i) : getLastCudaError() CUDA error: (%u) %s.\n", file, line,
            static_cast<unsigned int>(err), cudaGetErrorString(err));
    cudaDeviceReset();
    exit(EXIT_FAILURE);
  }
}
