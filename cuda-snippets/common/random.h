...
inline void FillRandomNumbers(std::vector<float>& array, float min = -1.0f,
...
// FillRandomNumbers 向输入序列中填充(min,max)之间的随机数
inline void FillRandomNumbers(std::vector<int>& array, int min = -1, int max = 1,
                       int64_t seed = 0L) {
  std::default_random_engine generator(seed);
...
inline void FillSequenceNumbers(std::vector<int>& array, int start = 0) {
  std::iota(array.begin(), array.end(), start);
}

template <typename T>
inline void PrintElements(const std::vector<T>& vec, size_t num = 0) {
  if (vec.empty()) {
    return;
  }
  if (num == 0) {
...
template <typename T>
inline void PrintElements2D(const std::vector<T>& matrix, int cols,
